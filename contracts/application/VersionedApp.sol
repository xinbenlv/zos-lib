...
contract VersionedApp is BaseApp {
  ...
  // function setPackageVersion(string name, string version) public onlyOwner {
  //   Package package = providers[name].package;
  //   require(address(package) != address(0), "The package was not found in the application");
  //   require(package.hasVersion(version), "The requested version must be registered in the given package");
  //   providers[name] = ProviderInfo(package, version);
  // }
}
